...
function getTabFilesName($files) {
     if ($files === null || empty($files['tmp_name'])) {
        return null;
    }
  $tabFilesName = "['";
  ...
  foreach ($files['tmp_name'] as $index => $tmpFilePath) {
      ...
      while (file_exists($_SERVER['DOCUMENT_ROOT'] . '/gess/pi/documents/' . $newFilename)) {
          $filenameParts = pathinfo($originalFilename);
          $newFilename = $filenameParts['filename'] . "_$counter." . $filenameParts['extension'];
          $counter++;
      }

      move_uploaded_file($tmpFilePath, $_SERVER['DOCUMENT_ROOT'] . '/gess/pi/documents/' . $newFilename);
  ...
  for ($i = 0; $i < count($role); $i++) {
    ...
    if($id_controle_interne[$i] =='0'){
      ...
      VALUES (:role, :nom_prenom, :age, :niveau_education, :anciennete, :num_tel, :idGess,:cp_cin)";
       $ran= rand(1, 500);
  $destinationPath =$_SERVER['DOCUMENT_ROOT'] . '/gess/pi/documents/';
...
if(isset($_POST['submit07'])){
        ...
        $presentation_proces_verbaux = $_FILES['presence_proces_verbaux'];
        $moyenne_presence_reunions =  $_FILES['moyenne_presence_reunions'];
        $absences_membres_justifiees =  $_FILES['absences_membres_justifiees'];
        $periodicite_reunions = $_FILES['periodicite_reunions'];
        $presentation_problemes_internes = $_POST['presentation_problemes_internes'];

        $fileFields = [
            'presence_proces_verbaux' => $presentation_proces_verbaux,
            'moyenne_presence_reunions' => $moyenne_presence_reunions,
            'absences_membres_justifiees' => $absences_membres_justifiees,
            'periodicite_reunions' => $periodicite_reunions,
        ];

        foreach ($fileFields as $fieldName => $fileValue) {
            if (isset($fileValue) && !empty($fileValue['name'][0])) {
                $sqlUpdate = "UPDATE pi_reunions_conseil_administration
        ...
        // Update the other non-file fields
        $sqlUpdate = "UPDATE pi_reunions_conseil_administration
                      SET nombre_reunions_annee_precedente = :nombre_reunions_annee_precedente,
                          presentation_problemes_internes = :presentation_problemes_internes
                      WHERE idGess = :idGess";

        $stmtUpdate = $conn->prepare($sqlUpdate);
        $stmtUpdate->bindParam(':nombre_reunions_annee_precedente', $nombre_reunions_annee_precedente);
        $stmtUpdate->bindParam(':presentation_problemes_internes', $presentation_problemes_internes);
        $stmtUpdate->bindParam(':idGess', $idGess);
        $stmtUpdate->execute();
    header("Location: ../../pages/info.php?id=$idGess");
}
